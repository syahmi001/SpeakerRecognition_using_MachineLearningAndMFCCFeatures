function result = KNNClassifier(trainedClassifier, featuresTest)
...
for idx = 1:length(Filenames)
    ...
    %result_file.ActualSpeaker = T.Label(1);
    %result_file.PredictedSpeaker = string(predictedLabel);
    result_file.PredictedSpeaker = char(predictedLabel);
    result_file.ConfidencePercentage = match;
    
    result_file.Filename = string(result_file.Filename);
    result_file.ActualSpeaker = string(result_file.ActualSpeaker);
    result_file.PredictedSpeaker = string(result_file.PredictedSpeaker);
    result_file.ConfidencePercentage = string(result_file.ConfidencePercentage);
